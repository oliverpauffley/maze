...
sidewinderCell maze c@(CellBoundaries _ down _ right (Coord (y, x))) =
  ...
  where
    ...
    pickDirection directions = do
      ...
      case ran of
        Down  -> handleDown maze c
        Right -> return $! linkCells maze (Coord (y, x), Coord (y, x + 1))
        _     -> undefined

    handleDown m currentCell = do
          let
            choice = getLinkedRowCells m currentCell ++ [location currentCell]
          newCell <- (trace $ (concatMap show (getLinkedRowCells m currentCell ))) (trace $ show $ location currentCell) (trace $ renderMaze maze) uniform choice
