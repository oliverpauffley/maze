...
startBoundaries (numCols, numRows) (Coord (y,x)) = CellBoundaries
...
initBlankSquareMaze size = Grid $ Map.fromList [(Coord (i, j), startSquare (Coord(i,j)))
                                        | i <- [0..size -1]
                                        , j <- [0..size -1]
                                        ]
  where startSquare = startBoundaries (size,size)
